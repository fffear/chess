module PossibleMoves
  ...
  def horizontal_move?(piece_location, king_position)
    (king_position > piece_location && (1..7).one? { |n| king_position == piece_location + n }) ||
    (piece_location > king_position && [-1, -2, -3, -4, -5, -6, -7].one? { |n| king_position == piece_location + n })
  end
  ...
  def blocked_upwards_and_to_the_right?(shift_factor, piece_location, king_position)
    piece_location < king_position && (piece_location + shift_factor..king_position - shift_factor).step(shift_factor).one? { |n| board.board[n].piece != " " }
  end

  def blocked_downwards_and_to_the_left?(shift_factor, piece_location, king_position)
    king_position < piece_location && (king_position + shift_factor..piece_location - shift_factor).step(shift_factor).one? { |n| board.board[n].piece != " " }
  end
